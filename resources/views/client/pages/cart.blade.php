...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#clearCart').click(function() {
            if (confirm(window.translations.confirm_clear_cart)) {
                window.Cart.clearCart();
                window.Cart.showToast(window.translations.cart_cleared);
            }
        });
        ...
        $('#applyPromo').click(function() {
            ...
            if (!promoCode) {
                window.Cart.showToast(window.translations.enter_promo_code, 'error');
                return;
            }
            ...
            if (promoCode.toUpperCase() === 'AISUKI10') {
                ...
                $('#discountAmount').text(`-${window.Cart.formatPrice(discount)}`).data('value', discount);
                
                // Cập nhật tổng cộng
                window.Cart.updateCartSummary();
                
                // Thông báo thành công
                window.Cart.showToast(window.translations.promo_applied);
            } else {
                // Mã không hợp lệ
                window.Cart.showToast(window.translations.invalid_promo, 'error');
            }
        });
    });
</script>
